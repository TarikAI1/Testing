# main_app.py
# ONLY Change: Applied sticky="nsew" to embed_listbox grid placement

import tkinter as tk
import tkinter.font as tkFont
# No ttk import needed (assuming this is based on the CTkTextbox preview version)
# If you reverted to the ttk.Treeview version, re-add `from tkinter import ttk`
...
class SteganoVFSApp:
    def __init__(self, master):
        self.master = master
        # Assuming reverted title is desired
        master.title("SteganoVFS")
        # Assuming reverted geometry/minsize is desired
        ...
        # Define Fonts (assuming this part was correct in the reverted version)
        ...
        # Instance Variables (assuming reverted state had QRNG switch)
        self.input_image_path = ""
        self.files_to_embed = []
        self.folders_to_embed = []
        self.use_qrng_flag = tk.BooleanVar(value=True)

        # GUI Structure (assuming layout from reverted state)
        master.columnconfigure(0, weight=1)
        # Adjust row configure based on the specific reverted version's preview widget
        # Assuming Textbox preview version's layout:
        ...
        preview_frame=customtkinter.CTkFrame(master, corner_radius=10); preview_frame.grid(row=4, column=0, sticky="nsew", padx=10, pady=5); preview_frame.columnconfigure(0, weight=1); preview_frame.rowconfigure(1, weight=1) # Textbox row expands
        ...
        # Files/Folders to Embed Selection
        ...
        listbox_frame.columnconfigure(0, weight=1); listbox_frame.rowconfigure(0, weight=1) # Let listbox row expand
        ...
        # *** THE FIX: Use sticky="nsew" ***
        self.embed_listbox.grid(row=0, column=0, sticky="nsew", padx=1, pady=1) # Fill cell N/S/E/W
        ...
        # Preview button also packed here in this version
        self.preview_button = customtkinter.CTkButton(embed_buttons_frame, text="Preview VFS", height=35, font=self.font_default, command=self.start_preview_thread, state=tk.DISABLED)
        self.preview_button.pack(side=tk.LEFT, padx=10)

        # VFS Preview Area (Using Textbox - Assuming this was the state after listbox fix)
        preview_inner_frame = customtkinter.CTkFrame(preview_frame, fg_color="transparent")
        preview_inner_frame.pack(fill="both", expand=True, padx=15, pady=(5,10))
        preview_inner_frame.columnconfigure(0, weight=1); preview_inner_frame.rowconfigure(0, weight=1) # Textbox row expands
        # No separate Preview button or label here in this reverted layout section
        self.preview_textbox = customtkinter.CTkTextbox(preview_inner_frame, font=self.font_preview, corner_radius=5, border_width=1, border_color=COLORS['border'], fg_color=COLORS['list_tree_bg'], text_color=COLORS['list_tree_fg'], wrap="none", state=tk.DISABLED)
        self.preview_textbox.grid(row=0, column=0, sticky="nsew", pady=(0, 5)) # Textbox takes whole inner frame
    ...
    def preview_vfs_threaded(self):
        ...
        try:
            ...
            self.master.after(0, self.populate_preview_textbox, extracted_items)
            self.master.after(0, self.update_status, f"Preview successful: {len(extracted_items)} items.", "success") # Original status update
             ...
             self.master.after(0, self.clear_preview_textbox)
    ...
    def populate_preview_textbox(self, items):
        ...
        try:
            ...
            print(f"[App Populate] Generated text length: {len(full_text)}")

            # Insert into textbox
            self.preview_textbox.configure(state=tk.NORMAL, text_color=COLORS['list_tree_fg'])
            self.preview_textbox.insert("1.0", full_text)
            print("[App Populate] Text inserted into textbox.")
            self.preview_textbox.configure(state=tk.DISABLED)

            # Debug check
            self.master.after(10, self._check_textbox_content)
    ...
    def _check_textbox_content(self):
        """Debug helper to check textbox content after population"""
        try:
            if hasattr(self, 'preview_textbox') and self.preview_textbox.winfo_exists():
                content = self.preview_textbox.get('1.0', tk.END).strip()
                print(f"[App Populate Debug] Textbox content get() length: {len(content)}")
                if not content: print("[App Populate Debug] Textbox appears empty after get()!")
        except Exception as e: print(f"[App Populate Debug] Error getting textbox content: {e}")
...
# *** Renamed function, simplified logic ***
def set_initial_window_size(window, width, height):
    """Attempts to set initial size and maximize vertically if possible."""
    try:
        print("[Window Setup] Setting initial geometry...")
        window.geometry(f"{width}x{height}")
        window.update_idletasks() # Ensure winfo is updated

        screen_height = window.winfo_screenheight()
        screen_width = window.winfo_screenwidth()
        print(f"[Window Setup] Screen: {screen_width}x{height}")

        # Simple vertical maximize attempt using geometry (most compatible)
        y_offset = 80 # Buffer for OS menus/docks
        target_height = screen_height - y_offset
        target_y = y_offset // 3
        # Use the initial width set earlier
        current_width = window.winfo_width() # Get actual width after initial geometry
        if current_width < width: # Fallback if initial geometry didn't stick
             current_width = width

        window.geometry(f"{current_width}x{target_height}+0+{target_y}")
        print(f"[Window Setup] Attempted vertical maximize geometry: {current_width}x{target_height}")

    except Exception as e:
        print(f"[Window Setup] Could not automatically adjust window size: {e}")


if __name__ == "__main__":
    root = customtkinter.CTk()
    app = SteganoVFSApp(root)
    # Attempt vertical maximize after mainloop starts
    root.after(150, lambda: set_initial_window_size(root, app.initial_width, app.initial_height))
    root.mainloop()